...
<template>
  <footer class="bg-primary pt-10 lg:pt-12">
    ...
    <!-- Bouton Scroll to Top avec GSAP -->
    <div ref="scrollToTopButton" class="fixed right-2 top-[95%] transform -translate-y-1/2 space-y-4 z-50 opacity-0">
      <div class="top-full right-0 mt-auto h-full flex justify-center md:justify-end z-50 pointer-events-none lg:pb-5 lg:pr-0 pr-2">
        <a href="#"
           ...
           class="rounded-xl h-12 w-12 flex items-center justify-center border-2 border-purple-600 bg-white
                  transition-transform hover:scale-110 pointer-events-auto shadow-lg hover:shadow-xl"
        >
...
// Fonction pour scroll fluide vers le top avec GSAP (alternative sans ScrollToPlugin)
const scrollToTop = () => {
...
onMounted(() => {
  // Animation des cartes locations
  ...
  ScrollTrigger.create({
    trigger: "body",
    start: "100vh top", // Apparaît après avoir scrollé 100vh (première section)
    end: "bottom bottom",
    onEnter: () => {
...
/* Style pour le bouton scroll to top */
.fixed a {
...
<!--            &lt;!&ndash; Sinon, image seule non cliquable &ndash;&gt;-->
<!--            <div v-else>-->
<!--              <img :src="none" alt="Carte" class="w-full h-32 object-cover cursor-wait rounded-md hover:opacity-80 transition-opacity duration-200"/>-->
<!--            </div>-->
<!--          </div>-->
